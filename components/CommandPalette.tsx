  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-zinc-900 border border-zinc-700 hover:bg-zinc-800 transition-colors text-sm text-zinc-400"
      >
        <span>Rechercher</span>
        <kbd className="text-xs bg-zinc-800 px-1.5 py-0.5 rounded border border-zinc-700 font-mono">⌘K</kbd>
      </button>
          ...
          <div className="relative w-full max-w-md mx-4">
            <Command className="bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl overflow-hidden">
              <Command.Input
                ...
                className="w-full px-4 py-3 bg-transparent text-white placeholder-zinc-500 outline-none border-b border-zinc-800 text-sm"
              />
              <Command.List className="p-2 max-h-64 overflow-y-auto">
                ...
                {commands.map((cmd) => (
                  <Command.Item
                    ...
                    className="flex items-center justify-between px-3 py-2 rounded-lg text-sm text-zinc-300 hover:bg-zinc-800 cursor-pointer aria-selected:bg-zinc-800"
                  >
                    <span>{cmd.label}</span>
                    <kbd className="text-xs text-zinc-600 font-mono">{cmd.shortcut}</kbd>
                  </Command.Item>
                ))}
              </Command.List>
            </Command>
